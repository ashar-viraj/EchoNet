...
// Cache filter option lists per mediatype to avoid repeated DISTINCT scans on large tables
const filterCache = new Map();
const FILTER_CACHE_TTL_MS = 10 * 60 * 1000; // 10 minutes

// Cache counts per filter signature to avoid recomputing COUNT(*) for the same query repeatedly
...
export async function getFilteredContent(mediatype, filters) {
    ...
    // Search filter on title/description using full-text with a LIKE fallback for safety
    if (search) {
  ...
  `;
    ...
    // Count total matching records with a short-lived cache
    ...
    // Fetch filters using precomputed lookup tables when present; fall back to live scan
...
async function fetchFilterOptions(mediatype) {
    ...
    try {
        ...
        // If lookup tables are populated, use them
    ...
    // Fallback: compute on the fly scoped to mediatype
