...
export default function TextPage() {
  ...
  const fetchData = async () => {
    ...
    // Add filters, handling arrays properly
    ...
    try {
      const res = await fetch(`/api/content/text?${params}`);
          ...
          {showFilters && (
            <aside className="w-full lg:w-80 space-y-6">
              <div className="bg-black/40 backdrop-blur-lg rounded-xl p-6 border border-purple-500/30 shadow-2xl">
                ...
                {/* Search */}
                <div className="mb-6">
                  <label className="block text-sm font-semibold mb-2 text-cyan-300">Search Title & Description</label>
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => {
                ...
                {/* Subject Tags */}
                <div className="mb-6">
                  <label className="block text-sm font-semibold mb-2 text-cyan-300">Subject Tags</label>
                ...
                {/* Downloads Range */}
                <div className="mb-6">
                  <label className="block text-sm font-semibold mb-2 text-cyan-300">Downloads Range</label>
                  ...
                  <div className="flex gap-2 mt-2">
                    {['0-100', '100-1000', '1k-10k', '10k+'].map(range => {
                      const [min, max] = range === '10k+' ? [10000, null] : 
                                         range === '1k-10k' ? [1000, 10000] :
                                         range === '100-1000' ? [100, 1000] : [0, 100];
                      return (
                        <button
                          key={range}
                          onClick={() => {
                            handleFilterChange('downloadsMin', min);
                            handleFilterChange('downloadsMax', max);
                          }}
                          className="text-xs px-2 py-1 bg-purple-600/30 hover:bg-purple-600/50 rounded border border-purple-400/30"
                        >
                          {range}
                        </button>
                      );
                ...
                {/* Size Range */}
                <div className="mb-6">
                  <label className="block text-sm font-semibold mb-2 text-cyan-300">File Size</label>
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      type="number"
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        {item.subject && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {Array.isArray(item.subject) ? item.subject.slice(0, 3).map((s, i) => (
                              <span key={i} className="text-xs px-2 py-1 bg-purple-600/30 rounded border border-purple-400/30">
                                {s}
                              </span>
                            )) : (
                              <span className="text-xs px-2 py-1 bg-purple-600/30 rounded border border-purple-400/30">
                                {item.subject}
                              </span>
                            )}
                          </div>
