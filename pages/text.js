...
export default function TextPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/content/text?${p.toString()}`);
          ...
          <aside className="w-full lg:w-80 space-y-4">
            <div className="bg-rustic-card border border-rustic rounded-2xl p-4 shadow-lg">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[11px] px-2 py-1 rounded-full bg-white/70 border border-rustic text-rustic-muted">Books</span>
              </div>
          ...
          <section className="flex-1 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                ...
                <h1 className="text-3xl font-semibold text-rustic-dark text-gradient-rustic">Books</h1>
            ...
            <div id="featured-audio" className="bg-rustic-card border border-rustic rounded-2xl p-5 shadow-lg">
              <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.12em] text-rustic-muted">Featured audiobooks</p>
                  <h2 className="text-xl font-semibold text-rustic-dark text-gradient-rustic">Listen or read these first</h2>
                </div>
                <span className="text-[11px] px-2 py-1 rounded-full bg-white/75 text-rustic-dark border border-rustic">
                  Handpicked titles
                </span>
              </div>
              <div className="grid gap-3">
                {featuredBooks.map((book) => (
                  <div id={`featured-${book.identifier}`} key={book.identifier} className="p-3 rounded-xl bg-white/85 border border-rustic flex flex-col md:flex-row md:items-center md:justify-between gap-3 shadow">
                    <div className="min-w-0">
                      <div className="font-semibold text-rustic-dark">{book.title}</div>
                      <div className="text-sm text-rustic-muted">{book.author}</div>
                      <p className="text-sm text-rustic-dark/80 line-clamp-2 mt-1">{book.description}</p>
                      <div className="text-xs text-rustic-muted mt-1">Language: {book.language}</div>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <button
                        className="px-3 py-2 bg-gradient-rustic text-white rounded-lg text-sm font-semibold hover:opacity-90 transition"
                        onClick={() => openLink(book.audioUrl, { ...book, url: `/text#featured-${book.identifier}` })}
                      >
                        Listen (Audiobook)
                      </button>
                      <button
                        className="px-3 py-2 bg-white/90 text-rustic-dark border border-rustic rounded-lg text-sm font-semibold hover:bg-rustic-card transition"
                      ...
                      <button
                        ...
                        className="px-3 py-2 bg-white/85 text-rustic-dark rounded-lg text-sm border border-rustic hover:bg-rustic-card transition"
                      >
                        Read aloud
                      </button>
                    </div>
                  ...
                  <div key={`${it.identifier}-${i}`} className="p-5 bg-rustic-card border border-rustic rounded-2xl hover:shadow-lg transition">
                    ...
                    <div className="mt-4">
                      ...
                      <button
                        ...
                        className="inline-flex items-center px-4 py-2 bg-white/85 text-rustic-dark rounded-lg text-sm font-semibold border border-rustic hover:bg-rustic-card ml-3 transition"
                      >
                        Read aloud
                      </button>
                    </div>
